...
mod tests {
    ...
    use crate::{circuit_builder::{tests::{RecursiveCircuit, NUM_PUBLIC_INPUTS_TEST_CIRCUITS}, CircuitLogic}, 
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        ...
        fn build_circuit(config: CircuitConfig, build_parameters: <LeafCircuit::<F, INPUT_SIZE> as CircuitLogic<F,D,0>>::CircuitBuilderParams) -> Self {
            ...
            let input_targets = <LeafCircuit::<F, INPUT_SIZE> as CircuitLogic<F,D,0>>::circuit_logic(
        ...
        fn generate_base_proof(&self, 
            circuit_set: &CircuitSet<F, C, D>,
            inputs: <LeafCircuit::<F, INPUT_SIZE> as CircuitLogic<F,D,0>>::Inputs
        ) -> Result<ProofWithPublicInputs<F,C,D>> {
            let mut pw = PartialWitness::<F>::new();
            <LeafCircuit<F, INPUT_SIZE> as CircuitLogic<F, D, 0>>::assign_input(&self.input_targets, inputs, &mut pw)?;
            CircuitSetDigest::from(circuit_set).set_circuit_set_target(&mut pw, &self.circuit_set_target);
        
            self.circuit_data.prove(pw)
        }

        fn wrap_base_proof(&self, base_proof: ProofWithPublicInputs<F, C, D>) -> Result<ProofWithPublicInputs<F, C, D>> {
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        fn build_circuit(config: CircuitConfig, circuit_set_size: usize) -> Self {
            let builder = UniversalVerifierBuilder::<F, D, NUM_PUBLIC_INPUTS>::new(config.clone(), circuit_set_size);
            ...
            let verifier_targets = builder.universal_verifier_circuit(&mut circuit_builder, &circuit_set_target);
            let proof_t = verifier_targets.get_proof_target();
            let input_targets = <RecursiveCircuit::<INPUT_SIZE> as CircuitLogic::<F,D,1>>::circuit_logic(
                &mut circuit_builder, [proof_t], ()
            );
        ...
        fn generate_base_proof(
            ...
            inputs: <RecursiveCircuit::<INPUT_SIZE> as CircuitLogic::<F,D,1>>::Inputs
                ...
                let mut pw = PartialWitness::<F>::new();
                self.verifier_targets.set_universal_verifier_targets(&mut pw, circuit_set, proof, verifier_data)?;
                <RecursiveCircuit::<INPUT_SIZE> as CircuitLogic::<F,D,1>>::assign_input(&self.input_targets, inputs, &mut pw)?;
                CircuitSetDigest::from(circuit_set).set_circuit_set_target(&mut pw, &self.circuit_set_target);
    ...
    fn test_circuit_with_universal_verifier() {
        ...
        let universal_verifier_circuit = CircuitWithUniversalVerifier::<F, C, D, INPUT_SIZE>::build_circuit(
            config.clone(), 
            CIRCUIT_SET_SIZE,
        );

        let circuit_set = CircuitSet::<F, C, D>::build_circuit_set(
            vec![
                base_circuit.get_circuit_data().verifier_only.circuit_digest,
                universal_verifier_circuit.get_circuit_data().verifier_only.circuit_digest,
            ]
        );

        let circuit_set_digest = CircuitSetDigest::from(&circuit_set);

        let base_proof = base_circuit.generate_proof(
            &circuit_set,
            (
        ...
        base_circuit.get_circuit_data().verify(base_proof.clone()).unwrap();

        let universal_verifier_proof = universal_verifier_circuit.generate_proof(
            &circuit_set, 
            &base_proof, 
        ...
        universal_verifier_circuit.get_circuit_data().verify(universal_verifier_proof.clone()).unwrap();

        assert_eq!(&universal_verifier_proof.public_inputs[NUM_PUBLIC_INPUTS..], circuit_set_digest.flatten().as_slice());
    ...
    fn negative_tests() {
        ...
        let mut base_circuit_variant = TestCircuitForUniversalVerifier::<F,C,D,INPUT_SIZE>::build_circuit(
            config.clone(),
            (1usize << 12, true)
        );

        let base_proof = base_circuit_variant.generate_base_proof(
            &circuit_set,
            (
            array::from_fn(|_| F::rand()),
            F::rand(),
        ...
        check_panic!(|| base_circuit.wrap_base_proof(base_proof.clone()), "wrapping base proof generated with wrong base circuit didn't fail");

        // use the proper wrap circuit
        println!("wrapping wrong base proof");
        let wrapped_proof = base_circuit_variant.wrap_base_proof(base_proof.clone()).unwrap();
        println!("wrong base proof wrapped");
        // universal verifier should detect that the circuit does not belong to circuit set
        let err = universal_verifier_circuit.generate_proof(
            &circuit_set, 
            &wrapped_proof, 
            &base_circuit_variant.get_circuit_data().verifier_only, 
            array::from_fn(|_| F::rand())
        ).unwrap_err(); 
        assert_eq!(format!("{}", err), "circuit digest not found", "universal verifier didn't fail when verifying a proof not belonging to circuit set");
        ...
        let wrong_circuit_set = CircuitSet::<F, C, D>::build_circuit_set(
            vec![
                base_circuit_variant.get_circuit_data().verifier_only.circuit_digest,
                universal_verifier_circuit.get_circuit_data().verifier_only.circuit_digest,
            ]
        );
        ...
        // not belonging to `circuit_set`
        {
            ...
            circuit_set.set_circuit_membership_target(&mut pw, 
                &universal_verifier_circuit.verifier_targets.circuit_set_membership, 
                base_circuit.get_circuit_data().verifier_only.circuit_digest,
            ).unwrap();
            ...
            CircuitSetDigest::from(&circuit_set).set_circuit_set_target(&mut pw, 
                &universal_verifier_circuit.circuit_set_target
            );
        ...
        circuit_data.verifier_only.circuit_digest = base_circuit
            .get_circuit_data()
            .verifier_only
            .circuit_digest;
        circuit_data.prover_only.circuit_digest = base_circuit
            .get_circuit_data()
            .prover_only
            .circuit_digest;
