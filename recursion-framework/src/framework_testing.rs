...
use plonky2::{field::extension::Extendable, hash::hash_types::RichField, iop::{target::Target, witness::{PartialWitness, WitnessWrite}}, 
    plonk::{circuit_builder::CircuitBuilder, circuit_data::{CircuitConfig, VerifierOnlyCircuitData}, 
    config::{AlgebraicHasher, GenericConfig, Hasher}, proof::{ProofWithPublicInputs, ProofWithPublicInputsTarget}}
};

use crate::{circuit_builder::{CircuitLogic, CircuitWithUniversalVerifier, CircuitWithUniversalVerifierBuilder}, 
...
> CircuitLogic<F,D,0> for DummyCircuit<NUM_PUBLIC_INPUTS>
{
    ...
    fn assign_input(&self, inputs: Self::Inputs, pw: &mut PartialWitness<F>) -> Result<()> {
        pw.set_target_arr(self.0.as_slice(), inputs.as_slice());
...
>
    ...
    where 
        ...
        [(); C::Hasher::HASH_SIZE]:,
    {
        CircuitWithUniversalVerifierBuilder::<F, D, NUM_PUBLIC_INPUTS>::new::<C>(config, circuit_set_size + 1)
    }
    ...
    [(); C::Hasher::HASH_SIZE]:,
{
    ...
    ) -> Self 
    {
        ...
        }).chain(once(dummy_circuit.get_verifier_data().circuit_digest)).collect::<Vec<_>>();
    ...
    /// bounded to `self`  
    pub fn generate_input_proofs<
        const NUM_VERIFIERS: usize,
    >(
        &self,
        public_inputs: [[F; NUM_PUBLIC_INPUTS]; NUM_VERIFIERS],
    ) -> Result<[ProofWithPublicInputs<F,C,D>; NUM_VERIFIERS]> {
        let input_proofs = public_inputs.into_iter().map(|inputs| {
    ...
    pub fn get_verifier_data_for_input_proofs<
        const NUM_VERIFIERS: usize,
    >(
        &self,
...
mod tests {
    ...
    use crate::{circuit_builder::tests::{RecursiveCircuit, NUM_PUBLIC_INPUTS_TEST_CIRCUITS}, framework::{prepare_recursive_circuit_for_circuit_set, 
        tests::{VerifierCircuit, VerifierCircuitFixed}, RecursiveCircuitsVerifierGagdet}};
        ...
        [(); C::Hasher::HASH_SIZE]:, 
    {
        ...
        let circuits = vec![prepare_recursive_circuit_for_circuit_set(&recursive_circuit)];
        ...
        let public_inputs_for_verified_proofs = array::from_fn(|_| 
            array::from_fn(|_| F::rand())
        );
        let proof = testing_framework.generate_proof_from_public_inputs(&recursive_circuit, public_inputs_for_verified_proofs, recursive_circuits_input).unwrap();

        assert_eq!(&proof.public_inputs[NUM_PUBLIC_INPUTS..], testing_framework.recursive_circuits.get_circuit_set_digest().flatten().as_slice());
    ...
    fn test_verifier_circuit_with_dummy_proofs() {
        ...
        let testing_framework = TestingRecursiveCircuits::<F,C,D,NUM_PUBLIC_INPUTS>::new(&circuit_builder, vec![]);

        // we build 2 circuits employing the `RecursiveCircuitsVerifierGadget`
        let circuit_builder = CircuitWithUniversalVerifierBuilder::<F,D, NUM_PUBLIC_INPUTS>::new::<C>(config.clone(), 2);

        let verifier_gadget = RecursiveCircuitsVerifierGagdet::new(config.clone(), &testing_framework.get_recursive_circuit_set());
        ...
        let verifier_circuit_fixed = circuit_builder.build_circuit::<C, 0, VerifierCircuitFixed<C,D,NUM_PUBLIC_INPUTS>>(
            (verifier_gadget, testing_framework.get_verifier_data_for_input_proofs::<1>()[0].clone())
        );
        ...
        let proof = recursive_framework_verifier_circuits.generate_proof(
            ...
            (
                ...
                testing_framework.generate_input_proofs::<1>(public_inputs).unwrap()[0].clone(),
