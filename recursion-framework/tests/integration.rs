...
use plonky2::{field::{extension::Extendable, types::PrimeField64}, hash::{hash_types::{RichField, NUM_HASH_OUT_ELTS}, poseidon::PoseidonHash}, iop::{target::Target, witness::{PartialWitness, WitnessWrite}}, plonk::{circuit_builder::CircuitBuilder, circuit_data::CircuitConfig, 
        config::{GenericConfig, GenericHashOut, Hasher, PoseidonGoldilocksConfig}, proof::ProofWithPublicInputsTarget}};
use recursion_framework::{circuit_builder::{CircuitLogic, CircuitWithUniversalVerifierBuilder}, framework::{prepare_recursive_circuit_for_circuit_set, RecursiveCircuitInfo, RecursiveCircuits, RecursiveCircuitsVerifierGagdet}, framework_testing::{new_universal_circuit_builder_for_testing, TestingRecursiveCircuits}};
...
impl<
    ...
    const D: usize,
    const ARITY: usize
>  CircuitLogic<F, D, ARITY> for ReduceCircuit<ARITY> {
    type CircuitBuilderParams = ();
    ...
    fn circuit_logic(
        ...
        let sum_target = verified_proofs.iter().fold(builder.zero(), |acc, &proof_t| 
            builder.add(acc, proof_t.public_inputs[0])
        );
        let hash_input = verified_proofs.iter().flat_map(|&proof_t|
            proof_t.public_inputs[1..NUM_PUBLIC_INPUTS].to_vec()
        ).collect::<Vec<_>>();
...
fn test_map_reduce_circuits() {
    ...
    let mut dataset_chunk_digests = dataset.chunks(INPUT_CHUNK_SIZE).map(|chunk| 
        PoseidonHash::hash_no_pad(chunk)
    ).collect::<Vec<_>>();
    while dataset_chunk_digests.len() != 1 {    
        let new_dataset_chunk_digests = dataset_chunk_digests.chunks(ARITY).map(|chunk| {
            let inputs = chunk.iter().flat_map(|hash| 
                hash.to_vec()
            ).collect::<Vec<_>>();
    ...
    let sum_of_even = dataset.into_iter().filter(|input| 
        input.to_canonical_u64() % 2 == 0
    ).reduce(|acc, el| acc + el).unwrap_or(F::ZERO);

    let map_proofs = dataset.chunks(INPUT_CHUNK_SIZE).map(|chunk| framework.generate_proof(
            &map_circuit, [], [], chunk.try_into().unwrap()
        )
    ).collect::<Result<Vec<_>>>().unwrap();
    let map_circuit_vd = map_circuit.get_verifier_data();
    let reduce_circuit_vd = reduce_circuit.get_verifier_data();
    let mut reduce_proofs = map_proofs.chunks(ARITY).map(|chunk| 
    ...
    while reduce_proofs.len() != 1 {
        let new_reduce_proofs = reduce_proofs.chunks(ARITY).map(|chunk| 
            framework.generate_proof(
                &reduce_circuit, 
                chunk.to_vec().try_into().unwrap(), 
    ...
    assert_eq!(root_proof.public_inputs[1..NUM_PUBLIC_INPUTS], dataset_digest.to_vec());

    reduce_circuit.circuit_data().verify(root_proof.clone()).unwrap();

    // build a circuit to recursively verify the root proof employing the `RecursiveCircuitVerifierGadget`
    let verifier_gadget = RecursiveCircuitsVerifierGagdet::<F, C, D, NUM_PUBLIC_INPUTS>::new(config.clone(), &framework);
    let mut builder = CircuitBuilder::<F, D>::new(config.clone());
    let proof_target = verifier_gadget.verify_proof_fixed_circuit_in_recursive_circuit_set(
        &mut builder, 
        reduce_circuit_vd,
    );
    ...
    assert_eq!(recursive_verifier_proof.public_inputs[1..NUM_PUBLIC_INPUTS], dataset_digest.to_vec());
...
fn test_reduce_circuit_with_testing_framework() {
    ...
    let test_public_inputs = array::from_fn(|_| {
        ...
        once(F::rand()).chain(hash.to_vec().into_iter()).collect::<Vec<_>>().try_into().unwrap()
    });
    let reduce_proof = testing_framework.generate_proof_from_public_inputs(
        &reduce_circuit, 
        test_public_inputs, 
        ()
    ).unwrap();
    ...
    let sum_of_even = test_public_inputs.iter().fold(F::ZERO, |acc, pub_input| 
        acc + pub_input[0]
    );
    let cumulative_hash = PoseidonHash::hash_no_pad(
            test_public_inputs.iter().flat_map(|pub_inp| 
            pub_inp[1..NUM_PUBLIC_INPUTS].to_vec()
        ).collect::<Vec<_>>().as_slice()
    );
    assert_eq!(reduce_proof.public_inputs[0], sum_of_even);
    assert_eq!(reduce_proof.public_inputs[1..NUM_PUBLIC_INPUTS], cumulative_hash.to_vec());
