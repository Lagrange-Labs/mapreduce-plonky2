...
/// The `KeyFetcher` skims through a [`Query`], and converts any reference to a
/// table by name to a virtual table generating on the fly a vTable-like
/// structure from a zkTable, while bubbling up to the top query the epoch and
...
/// The `Executor` skims through a [`Query`], and converts any reference to a
/// table by name to a virtual table generating on the fly a vTable-like
/// structure from a zkTable. For instance, SELECT [...] FROM some_table would
/// be converted to:
///
/// SELECT [...] FROM
///    (SELECT
