...
use anyhow::*;
use ryhope::{mapper_table_name, EPOCH, INCREMENTAL_EPOCH, KEY, PAYLOAD, USER_EPOCH, VALID_FROM, VALID_UNTIL};
use sqlparser::ast::{
    BinaryOperator, CastKind, DataType, Distinct, ExactNumberInfo, Expr, Function, FunctionArg, FunctionArgExpr, FunctionArgumentList, FunctionArguments, GroupByExpr, Ident, Join, JoinConstraint, JoinOperator, ObjectName, Query, Select, SelectItem, SetExpr, TableAlias, TableFactor, TableWithJoins, Value
};
...
// in the internal queries  generated by the visitors implemented in this module. 
...
fn block_range_table<C: ContextProvider>(
    settings: &ParsilSettings<C>,
    table: &ZkTable,
) -> TableWithJoins {
                ...
                subquery: Box::new(Query {
                    ...
                    body: Box::new(SetExpr::Select(Box::new(Select {
                        ...
                        projection: vec![
                            ...
                            SelectItem::UnnamedExpr(Expr::Identifier(Ident::new(INCREMENTAL_EPOCH)))
                        ],
                        into: None,
...
impl<'a, C: ContextProvider> KeyFetcher<'a, C> {
    ...
    // Internal implementation of `post_table_factor` for `KeyFetcher` visitor.
    // The `EPOCH_IS_INCREMENTAL` parameter affects whether the EPOCH column
    // being returned in the constructed query is an `INCREMENTAL_EPOCH` or a
    // `USER_EPOCH`, which depends on the context in which the query is executed.
    fn post_table_factor_internal<const EPOCH_IS_INCREMENTAL: bool>(
        &mut self, 
                ...
                let select_items =
                    ...
                    .chain(std::iter::once(
                        SelectItem::ExprWithAlias {
                            expr: if EPOCH_IS_INCREMENTAL {
                                Expr::Identifier(Ident::new(INCREMENTAL_EPOCH))
                            } else {
                                Expr::Identifier(Ident::new(USER_EPOCH))
                            },
                ...
                TableFactor::Derived {
                    ...
                    subquery: Box::new(Query {
                        ...
                        body: Box::new(SetExpr::Select(Box::new(Select {
                            ...
                            into: None,
                            from: vec![block_range_table(self.settings, &table)],
                            lateral_views: vec![],
                            prewhere: None,
                            selection: None,
                            group_by: GroupByExpr::Expressions(vec![], vec![]),
...
impl<C: ContextProvider> AstMutator for KeyFetcher<'_, C> {
    ...
    fn post_table_factor(&mut self, table_factor: &mut TableFactor) -> Result<()> {
        self.post_table_factor_internal::<true>(table_factor)
    }
}
...
impl<C: ContextProvider> AstMutator for ExecutorWithKey<'_, C> {
    ...
    fn post_table_factor(&mut self, table_factor: &mut TableFactor) -> Result<()> {
        ...
        key_fetcher.post_table_factor_internal::<false>(table_factor)
    }
