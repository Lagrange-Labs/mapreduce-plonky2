...
#[derive(Clone, Debug)]
struct MultisetHashingPointTarget<const N: usize> {
    /// X coordinate
    x: [Target; N],
    /// Y coordinate
    y: [Target; N],
...
fn hash_to_field_point_target<const N: usize>(hash: [Target; N]) -> MultisetHashingPointTarget<N> {
    todo!()
}
...
fn add_ext_values<F, const N: usize>(v1: [F; N], v2: [F; N]) -> [F; N]
where
    F: RichField + Extendable<N>,
{
...
fn add_ext_targets<F, const D: usize, const N: usize>(
    b: &mut CircuitBuilder<F, D>,
    t1: [Target; N],
    t2: [Target; N],
) -> [Target; N]
...
fn select_ext_target<F, const D: usize, const N: usize>(
    b: &mut CircuitBuilder<F, D>,
    is_t1: BoolTarget,
    t1: [Target; N],
    t2: [Target; N],
