...
mod tests {
    ...
    #[test]
    fn testrandom() -> Result<()> {
        let config = CircuitConfig::standard_recursion_config();
        let mut b = CircuitBuilder::<F, D>::new(config);
        let ar = 4;
        // Build the input and output targets.
        let input_targets = b.add_virtual_targets(ar);
        let mapped = b.map_to_curve_point(&input_targets);
        let zero_point = b.curve_zero();
        let output_target = b.curve_add(zero_point, mapped);
        ...
        // Generate random field values as inputs.
        let input_values = (0..ar)
            .map(|_| F::sample(&mut thread_rng()))
            .collect::<Vec<_>>();
        ...
        // Calculate the output point and check with proof.
        let mapped_point = map_to_curve_point(&input_values);
        let zero_point = Point::NEUTRAL;
        let expected_point = zero_point + mapped_point;
        let real_point = WeierstrassPoint {
            x: QuinticExtension(proof.public_inputs[..N].try_into().unwrap()),
            y: QuinticExtension(proof.public_inputs[N..N + N].try_into().unwrap()),
            is_inf: proof.public_inputs[N + N].is_nonzero(),
