...
fn prove_and_verify_length_extraction_circuit_for_pudgy() {
    ...
    let res = serde_json::from_str(PUDGY_STATE).unwrap();

    ProofQuery::verify_storage_proof(&res).unwrap();
    ...
    // what happens in circuit effectively we have to reverse since encoding is big endian on EVM
    // and our function is little endian based
    let slice = left_pad::<4>(&slice);
    let computed_length = convert_u8_to_u32_slice(&slice.into_iter().rev().collect::<Vec<u8>>())[0];
    assert_eq!(computed_length, length);
    ...
    let root: Vec<_> = convert_u8_to_u32_slice(&keccak256(&nodes[0]))
        .into_iter()
