...
impl PublicParameters {
    ...
    /// Displays circuit set digests
    pub fn display_digests(&self) -> std::collections::HashSet<Vec<u64>> {
        let mut new_set = std::collections::HashSet::<Vec<u64>>::new();
        println!(
            "Contract Digest: {:?}",
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Circuit set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }

        println!(
            "Length Digest: {:?}",
            self.length_extraction
                .get_circuit_set()
                .get_circuit_set_digest()
        );
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Length set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }

        println!(
            "Value extraction digest: {:?}",
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Value set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }

        println!(
            "Final extracion digest: {:?}",
        ...
        for leaf in final_mpt.leaves.iter() {
            println!("Final MT leaf: {:?}", leaf);
        }

        for digest in final_mpt.digests.iter() {
            println!("Final MP digest: {:?}", digest);
        }

        println!("Final extraction cap: {:?}", final_mpt.cap.0);
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Final set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Cells set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }
        println!(
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Rows set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }
        println!(
        ...
        for digest in self
            ...
            .keys()
        {
            println!("Block set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }
        println!(
        ...
        for digest in self
            ...
            .keys()
        {
            println!("IVC set hashmap digest: {:?}", digest);
            let new_value = new_set.insert(digest.clone());
            assert!(new_value);
        }
    ...
    pub fn check_digests(&self, new_set: &mut std::collections::HashSet<Vec<u64>>) {
        println!(
            "Contract Digest: {:?}",
            self.contract_extraction
                .get_circuit_set()
...
pub fn build_circuits_params() -> PublicParameters {
    ...
    let contract_ser = bincode::serialize(&contract_extraction).unwrap();
    let contract_len = contract_ser.len();
    let values_ser = bincode::serialize(&values_extraction).unwrap();
    let values_len = values_ser.len();
    let legth_ser = bincode::serialize(&length_extraction).unwrap();
    let length_len = legth_ser.len();

    let block_ser = bincode::serialize(&block_extraction).unwrap();
    let block_len = block_ser.len();

    let final_ser = bincode::serialize(&final_extraction).unwrap();
    let final_len = final_ser.len();

    let cells_ser = bincode::serialize(tree_creation.get_cells_params()).unwrap();
    let cells_len = cells_ser.len();

    let row_ser = bincode::serialize(tree_creation.get_row_params()).unwrap();
    let row_len = row_ser.len();

    let block_tree_ser = bincode::serialize(tree_creation.get_block_params()).unwrap();
    let bt_len = block_tree_ser.len();

    let ivc_ser = bincode::serialize(tree_creation.get_ivc_params()).unwrap();
    let ivc_len = ivc_ser.len();

    println!("After contract: {}", contract_len);
    println!("After value: {}", contract_len + values_len);
    println!("After length: {}", contract_len + values_len + length_len);
    println!(
        "After block: {}",
        contract_len + values_len + length_len + block_len
    );
    println!(
        "After final: {}",
        contract_len + values_len + length_len + block_len + final_len
    );
    println!(
        "After cells: {}",
        contract_len + values_len + length_len + block_len + final_len + cells_len
    );
    println!(
        "After row: {}",
        contract_len + values_len + length_len + block_len + final_len + cells_len + row_len
    );
    println!(
        "After block tree: {}",
        contract_len
...
mod tests {
    ...
    #[test]
    fn test_params_digests() {
        let _params = build_circuits_params();

        // let serialized = bincode::serialize(&params).unwrap();

        // let params_2 = build_circuits_params();

        // let serialized_2 = bincode::serialize(&params_2).unwrap();

        // for (i, (byte_1, byte_2)) in serialized.iter().zip(serialized_2.iter()).enumerate() {
        //     assert_eq!(*byte_1, *byte_2, "Bytes weren't equal at index: {}", i);
        // }
    }
}
