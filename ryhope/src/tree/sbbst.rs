...
async fn shift<const IS_EPOCH_TREE: bool, S: TreeStorage<Tree<IS_EPOCH_TREE>>>(s: &S) -> usize {
...
impl<const IS_EPOCH_TREE: bool> TreeTopology for Tree<IS_EPOCH_TREE> {
    ...
    async fn root<S: TreeStorage<Self>>(&self, s: &S) -> Option<NodeIdx> {
        let state = s.state().fetch().await;
        Some(if IS_EPOCH_TREE {
    ...
    async fn parent<S: TreeStorage<Self>>(&self, n: NodeIdx, s: &S) -> Option<NodeIdx> {
        let state = s.state().fetch().await;
        if IS_EPOCH_TREE {
    ...
    async fn lineage<S: TreeStorage<Self>>(&self, n: &NodeIdx, s: &S) -> Option<NodePath<NodeIdx>> {
        let state = s.state().fetch().await;
        if IS_EPOCH_TREE {
    ...
    async fn contains<S: TreeStorage<Self>>(&self, k: &NodeIdx, s: &S) -> bool {
        let state = s.state().fetch().await;
