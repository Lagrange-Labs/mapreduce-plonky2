...
use storage::{
    EpochKvStorage,
    FromSettings,
    PayloadStorage, RoEpochKvStorage, TransactionalStorage, TreeStorage, TreeTransactionalStorage,
    updatetree::{Next, UpdatePlan, UpdateTree}, view::TreeStorageView,
};
    ...
    > MerkleTreeKvDb<T, V, S>
{
    ...
    pub async fn try_fetch_with_context(&self, k: &T::Key) -> Option<(NodeContext<T::Key>, V)> {
        if let Some(ctx) = self.tree
            .node_context(k, &self.storage).await {
    ...
    pub async fn try_fetch_with_context_at(
        ...
        if let Some(ctx) = self.tree
            .node_context(k, &self.view_at(epoch)).await {
    ...
    pub async fn fetch_with_context_at(&self, k: &T::Key, epoch: Epoch) -> (NodeContext<T::Key>, V) {
    ...
    where <T as TreeTopology>::Key: Sync
    {
    ...
    > EpochKvStorage<T::Key, V> for MerkleTreeKvDb<T, V, S>
{
    async fn remove(&mut self, k: T::Key) -> Result<()> {
        self.dirty.extend(self.tree.delete(&k, &mut self.storage).await?);
    ...
    async fn update_with<F: Fn(&mut V)+ Send + Sync>(&mut self, k: T::Key, updater: F) {
        self.storage.data_mut().update_with(k.clone(), updater).await;
        self.dirty.insert(k);
    }
    ...
    > TreeTransactionalStorage<T::Key, V> for MerkleTreeKvDb<T, V, S>
{
    ...
    async fn commit_transaction(&mut self) -> Result<UpdateTree<T::Key>> {
        ...
        for k in self.dirty.iter() {
            if let Some(p) = self.tree.lineage(k, &mut self.storage).await {
                paths.push(p.into_full_path().collect::<Vec<_>>());
            }
        }
