...
use std::{
    collections::{HashMap, HashSet}, fmt::Debug, future::Future, hash::Hash, marker::PhantomData
};
use storage::{
    ...
    T::Key: Debug,
{
    ...
    pub async fn node_context_at(&self, k: &T::Key, epoch: UserEpoch) -> Option<NodeContext<T::Key>> {
        self.tree
            .node_context(k, &self.storage.view_at(epoch))
            .await
    }
    ...
    /// epoch.
    pub async fn diff_at(&self, epoch: UserEpoch) -> Option<UpdateTree<T::Key>> {
        if let Some(_) = self.current_epoch().await.ok().and_then(|current_epoch|
