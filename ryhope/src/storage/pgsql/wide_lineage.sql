...
WITH RECURSIVE descendance (is_core, key, parent, left_child, right_child, subtree_size, {VALID_FROM}, {VALID_UNTIL}, {PAYLOAD}, depth) AS (
     ...
     -- depth = 0
     SELECT ascendance_expanded.is_core, {zk_table}.key, parent, left_child, right_child, subtree_size, {VALID_FROM}, {VALID_UNTIL}, {PAYLOAD}, 0
       FROM {zk_table}
       ...
       RIGHT JOIN (
         ...
         SELECT is_core, core_keys.key, left_child, right_child, {VALID_FROM}, {VALID_UNTIL}
           FROM {zk_table}
         ...
         SELECT 0, parent.key, parent.left_child, parent.right_child, parent.{VALID_FROM}, parent.{VALID_UNTIL}
           ...
           WHERE current.key IN (parent.left_child, parent.right_child) AND NOT (parent.{VALID_UNTIL} <= $1 or parent.{VALID_FROM} >= $2)
         )
         SELECT
   ...
   SELECT 0, child.key, child.parent, child.left_child, child.right_child, child.subtree_size, child.{VALID_FROM}, child.{VALID_UNTIL}, child.{PAYLOAD}, depth + 1
     ...
     WHERE child.key IN (parent.left_child, parent.right_child) AND depth < $3 AND NOT (parent.{VALID_UNTIL} <= $1 or parent.{VALID_FROM} >= $2)
 ) SELECT
