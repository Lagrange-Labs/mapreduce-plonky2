...
WITH RECURSIVE descendance (is_core, key, parent, left_child, right_child, subtree_size, __valid_from, __valid_until, payload, depth) AS (
     ...
     -- depth = 0
     SELECT ascendance_expanded.is_core, {zk_table}.key, parent, left_child, right_child, subtree_size, __valid_from, __valid_until, payload, 0
       FROM {zk_table}
       ...
       RIGHT JOIN (
         ...
         SELECT 0, parent.key, parent.left_child, parent.right_child, parent.__valid_from, parent.__valid_until
           ...
           WHERE current.key IN (parent.left_child, parent.right_child) AND NOT (parent.__valid_until <= $1 or parent.__valid_from >= $2)
         )
         SELECT
   ...
   SELECT 0, child.key, child.parent, child.left_child, child.right_child, child.subtree_size, child.__valid_from, child.__valid_until, child.payload, depth + 1
     FROM {zk_table} child, descendance parent
     WHERE child.key IN (parent.left_child, parent.right_child) AND depth < $3 AND NOT (parent.__valid_until <= $1 or parent.__valid_from >= $2)
 ) SELECT
   ...
   GROUP BY (is_core, key, parent, left_child, right_child, subtree_size, __valid_from, __valid_until, payload)
