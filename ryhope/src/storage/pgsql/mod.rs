...
async fn delete_storage_table<const EXTERNAL_EPOCH_MAPPER: bool>(db: DBPool, table: &str) -> Result<()> {
    ...
    T::State: Sync + Clone,
{
    ...
    async fn create_tables(db: DBPool, table: &str, mapper_table: Option<String>) -> Result<()> {
    ...
    async fn commit_transaction(&mut self) -> Result<()> {
        ...
        let error_ctx = {
            ...
            cached_keys.extend(
                ...
                for k in cached_keys {
                    let node_value =
                        { self.tree_store.read().await.nodes_cache.get(&k).cloned() };
                    let data_value = {
                        self.tree_store
                            .read()
                            .await
