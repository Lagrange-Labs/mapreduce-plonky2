...
impl<K: Debug + Clone + Hash + Eq> UpdateTree<K> {
    ...
    pub fn from_paths<I: IntoIterator<Item = Vec<K>>>(paths: I, epoch: UserEpoch) -> Self {
    ...
    /// node.
    pub fn from_path(mut path: Vec<K>, epoch: UserEpoch) -> Self {
        path.reverse();
...
impl<K: Debug + Clone + Hash + Eq + Sync + Send> UpdateTree<K> {
    fn rec_build<'a, T: TreeTopology<Key = K>, S: TreeStorage<T>>(
        ...
        async {
            let context = t.node_context(current, nodes).await.unwrap().unwrap();
            let new_i = self.nodes.len();
    ...
    pub async fn from_tree<T: TreeTopology<Key = K> + Sync, S: TreeStorage<T>>(
        ...
        if let Some(root) = t.root(s).await.unwrap() {
...
impl<K: Clone + Hash + Eq + Sync + Send + std::fmt::Debug> UpdateTree<K> {
    ...
    pub fn from_map(epoch: UserEpoch, root: &K, nodes: &HashMap<K, NodeContext<K>>) -> Self {
        let mut r = Self::empty(epoch);
...
impl<T: Debug + Clone + Hash + Eq> UpdatePlan<T> {
    ...
    pub fn done(&mut self, k: &T) -> Result<(), RyhopeError> {
        let i = *self.t.idx.get(k).ok_or_else(|| RyhopeError::KeyNotFound)?;
