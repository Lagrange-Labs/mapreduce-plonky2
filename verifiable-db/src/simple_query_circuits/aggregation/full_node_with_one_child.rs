...
impl<const MAX_NUM_RESULTS: usize> FullNodeWithOneChildCircuit<MAX_NUM_RESULTS> {
        ...
        [(); MAX_NUM_RESULTS - 1]:,
    {
        ...
        // p.C == child.C
        b.connect_hashes(
            subtree_proof.computational_hash_target(),
            child_proof.computational_hash_target(),
        );

        // p.H_p == child.H_p
        b.connect_hashes(
            subtree_proof.placeholder_hash_target(),
...
mod tests {
    ...
    fn test_full_node_with_one_child_circuit(is_rows_tree_node: bool, is_left_child: bool) {
        ...
        if is_rows_tree_node {
            // child.I == p.I
            child_proof[index_value_range.clone()].copy_from_slice(subtree_pi.to_index_value_raw());
        }
