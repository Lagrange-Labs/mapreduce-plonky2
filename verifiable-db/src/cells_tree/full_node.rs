...
use mp2_common::{
    ...
    types::CBuilder, u256::CircuitBuilderU256, utils::ToTargets, CHasher, D, F,
};
use plonky2::{
...
impl FullNodeCircuit {
    pub fn build(b: &mut CBuilder, child_proofs: [PublicInputs<Target>; 2]) -> FullNodeWires {
        ...
        let individual_vd = b.add_curve_point(&[
            individual_vd,
            p1.individual_values_digest_target(),
            p2.individual_values_digest_target(),
        ]);
        let multiplier_vd = b.add_curve_point(&[
            multiplier_vd,
            p1.multiplier_values_digest_target(),
            p2.multiplier_values_digest_target(),
        ]);
        let individual_md = b.add_curve_point(&[
            individual_md,
            p1.individual_metadata_digest_target(),
            p2.individual_metadata_digest_target(),
        ]);
        let multiplier_md = b.add_curve_point(&[
            multiplier_md,
            p1.multiplier_metadata_digest_target(),
            p2.multiplier_metadata_digest_target(),
        ]);
