...
use crate::api::PublicParameters as PreprocessingParameters;
...
impl<const BLOCK_DB_DEPTH: usize, const L: usize> PublicParameters<BLOCK_DB_DEPTH, L> {
    ...
    pub fn build(preprocessing_params: &PreprocessingParameters<BLOCK_DB_DEPTH>) -> Self {
        ...
        let revelation = revelation::Parameters::build(
            ...
            preprocessing_params.get_block_db_circuit_set(),
            preprocessing_params.get_block_db_vk(),
        );
        Self {
            storage,
            state,
