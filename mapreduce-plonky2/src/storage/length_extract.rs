...
where
    ...
    [(); DEPTH - 1]:,
{
    pub fn new(slot: u8, nodes: Vec<Vec<u8>>) -> Self {
...
pub const MAX_DEPTH_TRIE: usize = 5;
...
mod tests {
    ...
    use crate::{
        ...
        eth::{get_mainnet_url, get_sepolia_url, left_pad, ProofQuery, StorageSlot},
    ...
    fn test_length_extract_circuit() {
        ...
        let test_circuit = LengthTestCircuit::<DEPTH, NODE_LEN> {
            base: ArrayLengthExtractCircuit::new(test_data.slot, test_data.nodes),
        };
    ...
    async fn test_length_extract(t: LengthTestData) -> Result<()> {
        ...
        let test_circuit = LengthTestCircuit::<DEPTH, NODE_LEN> {
            base: ArrayLengthExtractCircuit::new(slot, nodes),
        };
        let proof = run_circuit::<F, D, C, _>(test_circuit);
