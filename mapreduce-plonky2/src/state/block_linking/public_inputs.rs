...
use super::{storage_inputs::StorageInputs, BlockLinkingWires};
...
impl<T> PublicInputs<T> {
        ...
        [(); DEPTH - 1]:,
    {
        ...
        cb.register_public_inputs(storage_inputs.a());
        cb.register_public_inputs(storage_inputs.d());
        cb.register_public_inputs(storage_inputs.m());
        cb.register_public_inputs(storage_inputs.s());

        // Only expose the equivalent storage tree root here, NOT the one from
        // blockchain.
        cb.register_public_inputs(storage_inputs.merkle_root());
    }
