...
use super::{account::public_inputs::PublicInputs as AccountPubInputs, BlockLinkingWires};
use crate::{
...
impl<'a, T: Clone> BlockLinkingInputs<'a, T> {
    pub fn register<F, const D: usize, const BLOCK_LEN: usize>(
        cb: &mut CircuitBuilder<F, D>,
        wires: &BlockLinkingWires<BLOCK_LEN>,
        account_inputs: &AccountPubInputs<Target>,
    ) where
        F: RichField + Extendable<D>,
        [(); PAD_LEN(BLOCK_LEN)]:,
    {
        wires.hash.output_array.register_as_public_input(cb);
        cb.register_public_input(wires.number.0);
        wires.parent_hash.register_as_public_input(cb);
        account_inputs
        ...
        cb.register_public_input(account_inputs.mapping_slot());
        cb.register_public_input(account_inputs.length_slot());

        // Only expose the equivalent storage tree root here, NOT the one from
        // blockchain.
        cb.register_public_inputs(account_inputs.lpn_root_info());
    }
